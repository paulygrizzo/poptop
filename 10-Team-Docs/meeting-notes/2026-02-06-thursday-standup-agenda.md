## PopTop Thursday Standing Call - Agenda
**Date:** Thursday, February 6, 2026
**Attendees:** Paul Giarrizzo, Alex Munn, Brian Williams, Whitney Sanchez (invited)
**Duration:** 30-45 minutes
**Call Type:** Reboot / Realignment

---

## Action Items (from this call)

| AR# | Owner | Action | Due | Status |
|-----|-------|--------|-----|--------|
| AR-001 | Whitney | Reach out for follow-up call | WW07 (Feb 10-14) | Open |
| AR-002 | Brian | Have 1st prototype pieces ready | WW06.2 (Feb 6-7) | Open |
| AR-003 | Brian | Drop CAD files into Engineering folder | WW06.2 | Open |
| AR-004 | Paul | Set up recurring Thursday calendar invite | WW07 | Open |
| AR-005 | Paul | Authorize Jeff Johnson trademark filing ($1,300) | Feb 7 | Open |
| AR-006 | Paul | Share Google Sheets Command Center with team | Feb 7 | Open |
| AR-007 | Alex | Confirm capital commitment for Phase 1-2 | Feb 13 | Open |
| AR-008 | Alex | Register domain (drinkpoptop.com or getpoptop.com) | Feb 13 | Open |

---

## Quick Agenda Overview
- **Welcome & Reboot** — Framing for Fall 2026 launch
- **Team Check** — Confirm everyone's still in, same roles
- **Engineering Status** — CAD progress, path to design freeze
- **Timeline** — Review 7-month execution plan
- **Trademark Decision** — Authorize $1,300 filing?
- **Action Items** — Assign owners for next 2 weeks
- **Logistics** — Set up recurring Wednesday calls

---

## 1. Welcome & Reboot Framing (5 min)
**Lead: Paul**

- Acknowledge the gap since last active push
- Restate the opportunity: collegiate tailgating market, licensing-first model
- New target: **Launch before Fall 2026 College Football season (Sep 1, 2026)**
- This call is about getting aligned, re-committed, and moving

---

## 2. Team Commitment Check (10 min)
**Lead: Paul**

Go around -- each person confirms:
- Are you still in? Same role, same capacity?
- Any changes in availability or circumstances?
- What does your bandwidth look like for the next 7 months?

**Key roles to reconfirm:**
| Name | Role | Question |
|------|------|----------|
| Alex | Co-Founder | Capital availability -- can we fund Phase 1-2 ($20-30K)? |
| Brian | Engineer | Can you commit to CAD/prototyping? Timeline for design freeze? |
| Whitney | Operations | Available for task execution, social media, comms? |

---

## 3. Engineering & Product Status (10 min)
**Lead: Brian**

- Where do the CAD designs stand?
  - Reference: existing STEP/STL/SolidWorks files on file 
	  - Everything is in Solidworks and have STEP/STL files
  - CenterMag assembly, Can Holder, Container Top, Lid, Tube
	  - All is designed
- What's needed to get to a design freeze?
	- Get thru prototype (likely multiple iterations)
		- Springs (Vulcan Springs) - Will need to send a prototype
		- Targeting to have pieces for 1st Prototype prior in WW06.2 - Brian
		- Drop files into Eng. Folder - Brian
- Dispensing mechanism design -- any progress or ideas?
	- No concern/working on design
- DFM considerations from what you've seen so far?
	- May need to print some parts
	- Considering injection molding (**16-20 wk LT** for molds, etc.)
	- Blow molding or roto molded - for exterior
	- Extrusion (potentially PVC/aluminum/etc)
	- Cooling element
- What do you need from the team to move forward?

---

## 4. Revised Timeline Discussion (5 min)
**Lead: Paul**

Present the reset execution plan targeting Fall 2026 launch:

| Phase   | Focus                        | Target                               |
| ------- | ---------------------------- | ------------------------------------ |
| Phase 0 | Reboot & Foundation          | Feb 2026 (now)                       |
| Phase 1 | Design & Prototyping         | Feb - Apr 2026                       |
| Phase 2 | Brand Identity & IP          | Feb - Apr 2026 (parallel)            |
| Phase 3 | Manufacturing & Supply Chain | Apr - Aug 2026                       |
| Phase 4 | Licensing (CLC/Fanatics)     | Apr - Jul 2026 (6-12 wks post-proto) |
| Phase 5 | Go-To-Market Prep            | Jun - Aug 2026                       |
| Phase 6 | Launch & Operations          | Sep 1, 2026                          |

**Decision needed:** Does this pace work for everyone? This is aggressive.

---

## 5. Trademark Update & Decision (5 min)
**Lead: Paul**

Quick update: Jeff Johnson (IP Law USA) completed a professional trademark search in April 2025.

**His findings:**
- Two potentially relevant marks found ("Pop Top" Class 21 water bottles, "Top Pop" Class 32 soft drinks)
- **Neither is a showstopper** -- our goods/services are reasonably different
- **His recommendation: proceed with filing**
- **Cost: $1,300** (includes gov fees for 1 class; $350/class extra if USPTO requires additional)

**Status:** Jeff's firm has been waiting since June 2025 for our go-ahead to file.

**Decision needed:** Do we authorize Jeff to file the POPTOP trademark? ($1,300)

---

## 6. Immediate Action Items (5 min)
**Lead: Paul**

Assign owners and due dates for the next 2 weeks:

### Paul
- [ ] Authorize Jeff Johnson to file trademark (if agreed on call)
- [ ] Share Google Sheets Command Center with team (Editor access)
- [ ] Share updated execution plan v2.1 + user guide with team
- [ ] Contact Ross Munn separately with update

### Brian
- [ ] Review existing CAD files, assess current state by Feb 13
- [ ] Confirm engineering timeline for design freeze (target: Apr 24, 2026)
- [ ] Sign Product Development & Royalty Agreement (if not already done)

### Alex
- [ ] Confirm capital commitment for Phase 1-2
- [ ] Register best available domain (drinkpoptop.com or getpoptop.com)
- [ ] Review updated execution plan

### Whitney
- [ ] Review assigned tasks in Google Sheets Command Center
- [ ] Begin social media handle research
- [ ] Set up team communication channel (group text / Slack / Discord)

---

## 7. Standing Call Logistics (2 min)
**Lead: Paul**

- **Proposal:** Move recurring call to Thursdays starting Feb 11
- Confirm time slot
- Confirm format (video call, phone, etc.)
- Set up recurring Thursday calendar invite
- Agree on communication between calls (text group, email, etc.)

---

## Key Decisions to Make on This Call

1. **Team commitment** -- Is everyone still in, same roles?
2. **Timeline** -- Fall 2026 (Sep 1) target agreed?
3. **Capital** -- Can we fund initial phases ($4-10K near-term)?
4. **Brian's agreement** -- Signed or path to signing?
5. **Trademark** -- Authorize Jeff Johnson to file ($1,300)? Attorney recommends proceeding.
6. **Standing call day** -- Move to Wednesdays starting Feb 11?
7. **Communication** -- How do we stay in touch between calls?

---

## Trademark Situation (Brief -- Paul to present)

Jeff Johnson (IP Law USA) completed a professional trademark search in April 2025. Found two potentially relevant marks but **recommends proceeding** -- neither is a showstopper for our beverage dispenser goods/services. His firm is ready to file for **$1,300**. They've been waiting for our green light since June 2025.

---

## Reference Materials
- Business Plan: `01-Business-Plan/final/PopTop-Business-Plan-v1.pdf`
- Execution Plan v2: `10-Team-Docs/PopTop-Execution-Plan-v2.pdf`
- Trademark Research: `02-Legal-IP/trademark-domain-research-2025-02-03.md`
- CAD Files: `~/Desktop/Pop Top Drawings/`
- Tracker: [Google Sheet](https://docs.google.com/spreadsheets/d/1JJjYJitZ_jg7ui0JqdID7MtuhGamyZxS1wNbDVw0cp4/)

---

*Prepared by Paul Giarrizzo | February 4, 2026*
